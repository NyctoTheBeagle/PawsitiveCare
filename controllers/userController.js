    ...
    try {
      ...
      const availableDates = await Promise.all(
        targetDates.map(async (date) => {
          ...
          return {
            date,
            isAvailable: appointments.length < 10,
          };
        })
      );
...
const editAppointmentController = async (req, res) => {
    try {
        ...
        const updatedData = req.body; // Data with updated appointment details
        ...
        const updatedAppointment = await appointmentModel.findByIdAndUpdate(appointmentId, updatedData, { new: true });
